...
interface ReverseGameState {
  phase: "loading" | "guessing" | "result" | "error"
  original: {
    name: string
    url: string
  } | null
...
export default function ReverseGame({ onBackToMenu }: ReverseGameProps) {
  ...
  const retryCurrentOperation = () => {
    if (gameState.phase === "error") {
      // Reset error state and retry the current round
      setGameState((prev) => ({
        ...prev,
        phase: "loading",
        error: undefined,
  ...
  const startNewRound = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Runway API failed with status ${response.status}`)
      }
      ...
      if (!transformedImage) {
        throw new Error("No output from Runway API")
      }
      ...
      handleError(error as Error, "image generation")
    }
  }
